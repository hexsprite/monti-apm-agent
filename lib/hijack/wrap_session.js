    ...
    if (true) {
      ...
      if (msg.msg == 'method' || msg.msg == 'sub') {
        ...
        if (stringifiedParams.length > MAX_PARAMS_LENGTH) {
          stringifiedParams = `Monti APM: params are too big. First ${MAX_PARAMS_LENGTH} characters: ${stringifiedParams.slice(
            0,
            MAX_PARAMS_LENGTH
          )}`;
        }
        ...
        let waitEventId = Kadira.tracer.event(
          kadiraInfo.trace,
          'wait',
          {},
          kadiraInfo
      ...
      if (kadiraInfo) {
        if (msg.error) {
          ...
          if (kadiraInfo && kadiraInfo.currentError) {
            ...
            error = _.pick(kadiraInfo.currentError, [
              'message',
              'stack',
              'details',
            ]);
